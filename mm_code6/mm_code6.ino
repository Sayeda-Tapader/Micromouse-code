...
void setMotors(int dirLeft, int dirRight, int speed) {

    // Debugging: Print out the signals for the left motor
    Serial.print("Left Motor - Direction: ");
    Serial.print(dirLeft == 1 ? "Forward" : "Reverse");
    Serial.print(", Speed: ");
    Serial.println(speed);

    //NOTE: for motor_R to go forward we have to set it to LOW
    //as its the opposite of motor_L for some reason. motor_L is HIGH for forward

    // Set direction and speed for the left motor
    if (dirLeft == 1) {
        fast_write_pin(DIR_MOTOR_L, HIGH); // Forward
    } else if (dirLeft == -1) {
        fast_write_pin(DIR_MOTOR_L, LOW); // Reverse
    }

    // Set direction and speed for the right motor
    if (dirRight == 1) {
        fast_write_pin(DIR_MOTOR_R, LOW); // Forward
    } else if (dirRight == -1) {
        fast_write_pin(DIR_MOTOR_R, HIGH); // Reverse
    }
...
void motorPID(int setPoint, float kp, float ki, float kd){
  ...
  int dirRight = 1;
  if (u < 0) {
    dirLeft = -1; // Move backward
    dirRight = -1; 
  } else {
    dirRight = 1; // Move forward
    dirLeft = 1; 
  }
  ...
  setMotors(dirLeft, dirRight, speed);
  prevError = 0;
}
//==============================================================================================
...
void loop() {
  ...
  // Read dip switch value to determine if the robot should start moving
  int dipSwitch = analogRead(DIP_SWITCH);
  ...
  // If the switch is on, start moving the robot
  if(switchOn){
    // Delay to ensure safe start
     delay(500); // Wait half a second after pressing the button to actually start moving, safety first!
    // Call motorPID function to control robot movement
    int setPoint = 1000;
    float kp = 1;
    float ki = 0.1;
    float kd = 0.001;
    motorPID(setPoint, kp, ki, kd);
    // Print setpoint and encoder position for debugging
